...
import { getStatus, connect, disconnect, sendText, sendVoice, generateVoice } from '@/lib/wa-client';
...
For "call": When user says "call [name]" or "phone [name]"
For "voice_message": When user says "voice message to [name]" or "send voice message to [name]" or "tell [name]"
For "text_message": When user says "text [name]" or "message [name]" or "send message to [name]" or "whatsapp [name]"
For "chat": Everything else (normal conversation)

Always match contact names from the provided contacts list. If the user says "call Mama", find "Mama" in contacts.`;
...
Rules:
- Keep it short, friendly, and professional (1-2 sentences)
...
- Return ONLY the message text, no explanations`;
  ...
  return response.text || `Hi ${contactName}, this is Abdullah's AI assistant Zoe. Hope you're doing well!`;
}

// Initiate Twilio call
async function initiateTwilioCall(to: string, contactName: string, message: string) {
  ...
  const client = twilio.default(twilioAuth.sid, twilioAuth.token);
  
  const appUrl = process.env.APP_URL || 'http://localhost:3000';
  const twimlUrl = `${appUrl}/api/twilio/twiml?contactName=${encodeURIComponent(contactName)}&message=${encodeURIComponent(message)}`;
  
  const call = await client.calls.create({
    to: to,
    from: twilioAuth.whatsapp || '+14155238886',
    url: twimlUrl,
  });
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      switch (intent.action) {
        case 'call': {
          ...
          try {
            ...
            const call = await initiateTwilioCall(
              contact.phone,
              contact.name,
              intent.message || `Connecting you to ${contact.name}...`
            );
        ...
        case 'chat':
        default: {
          // Normal chat response - get Gemini for chat
          const apiKey = await getGeminiKey();
          const ai = new GoogleGenAI({ apiKey });
